    ...
    return (
      <header
        className={`fixed w-full z-50 flex justify-between items-center px-8 py-4 border-b ${
          ...
          currentY < 1300 ? 'bg-transparent' : 'bg-white/30 backdrop-blur-sm backdrop-saturate-50'
        }  `}>
        <h1
            ...
            <li
              ...
              <div
                className={`absolute top-[160%] w-full h-2 ${
                  section.to === currentSection && currentY > 1300 ? 'bg-black' : ''
                } ${section.to === currentSection && currentY < 1300 ? 'bg-white' : ''} `}></div>
            </li>
          ))}
        </ul>
      </header>
